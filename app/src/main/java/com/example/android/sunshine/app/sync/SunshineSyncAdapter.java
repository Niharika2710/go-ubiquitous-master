...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            if ( cVVector.size() > 0 ) {
                ...
                notifyWeather();
                if (googleApiClient != null) {
                    notifyWearable(cVVector.elementAt(0));
                }
            }
    ...
    private void notifyWearable(ContentValues todayWeatherValues) {
        int weatherId = (int) todayWeatherValues.get(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID);
        double minTemp = (double) todayWeatherValues.get(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP);
        double maxTemp = (double) todayWeatherValues.get(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP);

        PutDataMapRequest putDataMapRequest = PutDataMapRequest.create(UPDATE_DATA_WEAR_URI);
        DataMap map = putDataMapRequest.getDataMap();

        map.putLong("time", new Date().getTime());
        map.putString("minTemp", Utility.formatTemperature(getContext(), minTemp));
        map.putString("maxTemp", Utility.formatTemperature(getContext(), maxTemp));
